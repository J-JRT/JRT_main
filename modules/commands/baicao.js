...
module.exports.handleEvent = async ({ event, api, Users }) => {
	...
	if (body.indexOf("ready") == 0) {
		...
		else return api.sendMessage(`👤 ${name} Đã sẵn sàng lật bài\n\nCòn lại: ${values.player.length - values.ready} condi chưa dám lật bài`, event.threadID);
	}
...
module.exports.run = async ({ api, event, args }) => {
	...
	switch (args[0]) {
		...
		case "-c": {
			...
			return api.sendMessage("» Đã tạo bàn bài cào! «\n\n👉Tham gia, nhập /baicao join\n👉Khi đủ ng, nhập /baicao start", threadID, messageID);
		}
		...
		case "-s": {
			...
			values.start = 1;
			return api.sendMessage("» Bắt đầu cuộc chơiii «\n\nĐể chia bài, nhập 'chia bài'", threadID, messageID);
		}
