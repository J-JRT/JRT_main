module.exports.config = {
  ...
  usages: "shortaudio a => [reply ghi âm /link ghi âm (nếu nhiều link ngăn cách bằng " | ")]",
  cooldowns: 5,
  dependencies: {
        "fs-extra": ""
    }
      ...
      if (getThread.some(item => item.in == event.body)) {
        ...
        if (typeof shortOut == "string") {
          ...
          if (shortOut.indexOf(" | ") !== -1) {
            ...
            fs.writeFileSync( __dirname + `/cache/shortaudio${i}.mp3`, Buffer.from(shortimage, "utf-8"));
            path.push(fs.createReadStream( __dirname + `/cache/shortaudio${i}.mp3`));
            //api.sendMessage({attachment: fs.createReadStream( __dirname + `/cache/shortaudio${i}.mp3`)}, event.threadID, () => fs.unlinkSync(__dirname + `/cache/shortaudio${i}.mp3`));                 
            }
            var mainpath = [...path];
            api.sendMessage({attachment: mainpath}, event.threadID);  
            for (let i = 0; i < mainpath.length; i++) {
            fs.unlinkSync(__dirname + `/cache/shortaudio${i}.mp3`)
            }           
            //return api.sendMessage(`${arrayOut[Math.floor(Math.random() * arrayOut.length)]}`, event.threadID);
          }
          else {
            if (!regex.test(`${shortOut}`)) return api.sendMessage(`Item ${shortOut} không phải là url`, event.threadID, () => fs.unlinkSync(__dirname + `/cache/shortaudio.mp3`));
            let shortimage = (await axios.get( `${shortOut}`, { responseType: "arraybuffer" })).data;
            fs.writeFileSync( __dirname + `/cache/shortaudio.mp3`, Buffer.from(shortimage, "utf-8"));
            api.sendMessage({attachment: fs.createReadStream( __dirname + `/cache/shortaudio.mp3`)}, event.threadID);             }
        }
  ...
  if (content.indexOf(`xóa`) == 0 || content.indexOf(`xoá`) == 0|| content.indexOf(`del`) == 0) {
    ...
    return fs.readFile(__dirname + "/cache/shortaudio.json", "utf-8", (err, data) => {
      ...
      if (allData.some(item => item.id == threadID)) {
        ...
        getThread.forEach(item => msg = msg + item.in + ' -> ' + (typeof item.out == "object" ? `${item.out.length} audio` : item.out.indexOf(" | ") !== -1 ? `${item.out.split(" | ").length} audio` : "1 audio") + '\n');
      }
