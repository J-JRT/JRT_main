    ...
    case 'only':
        ...
        case '-sp': {
            ...
            if (config.ndhOnly == false) {
                config.ndhOnly = true;
                api.sendMessage(`[⚜️] Bật thành công chỉ Admin Support mới dùng được bot`, threadID, messageID);
            } else {
                config.ndhOnly = false;
                api.sendMessage(`[⚜️] Tắt thành công chỉ Admin Support mới dùng được bot`, threadID, messageID);
            }
                writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                break;
              }
        case 'ibrieng': {
          if (permssion != 3) return api.sendMessage("[⚜️] Xin lỗi! lệnh này chỉ admin bot mới dùng được", threadID, messageID);
           if (config.ibrieng == false) {
                config.ibrieng = true;
                api.sendMessage("[⚜️] Bật thành công ibrieng với bot", threadID, messageID);
            } else {
                config.ibrieng = false;
                api.sendMessage("[⚜️] Tắt thành công ibrieng với bot", threadID, messageID);
            }
                writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                break;
        }
        case 'dangky': {
          if (permssion != 3) return api.sendMessage("[⚜️] Xin lỗi! lệnh này chỉ admin bot mới dùng được", threadID, messageID);
           if (config.dangky == false) {
                config.dangky = true;
                api.sendMessage("[⚜️] Bật thành công đăng ký dùng bot", threadID, messageID);
            } else {
                config.dangky = false;
                api.sendMessage("[⚜️] Tắt thành công đăng ký dùng bot", threadID, messageID);
            }
                writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                break;
        }
